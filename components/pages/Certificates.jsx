...
import { Card, Typography, Button, Tag, Space, Row, Col, Modal, Tooltip, Divider, Result, Skeleton, message } from 'antd';
import { 
...
export default function Certificates() {
  ...
  const [status, setStatus] = useState('locked'); // 'active' | 'locked'
  ...
  useEffect(() => {
    const fetchCertificateStatus = async () => {
      try {
        const res = await fetch('/api/v1/certificate/status');
        if (res.ok) {
  ...
  const handleDownload = () => {
    window.location.href = '/api/v1/certificate/download';
  };

  const handleShare = () => {
    if (certData?.certificateNumber) {
      const url = `${window.location.origin}/verify/${certData.certificateNumber}`;
      navigator.clipboard.writeText(url)
        .then(() => message.success('Verification URL copied to clipboard!'))
        .catch(() => message.error('Failed to copy link.'));
    }
  };

  // Render Realistic Certificate Canvas
    ...
    <div className={`relative w-full ${isModal ? 'aspect-[1.414/1]' : 'aspect-[1.414/1]'} bg-gradient-to-br from-[#fdfbfb] to-[#ebedee] border-[12px] border-slate-100 shadow-[inset_0_0_40px_rgba(0,0,0,0.05)] flex flex-col items-center justify-center p-8 text-center`}>
      ...
      <div className="relative z-10 w-full max-w-lg mx-auto flex flex-col items-center">
        ...
        <div className="w-full border-b border-slate-400 pb-2 mb-4">
          <Title level={isModal ? 1 : 2} className="!text-blue-900 !m-0 !font-serif italic">{certData?.studentName || "Student Name"}</Title>
        </div>
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 bg-blue-50/50 min-h-full">
      ...
      {loading ? (
        ...
        <Card className="rounded-2xl border-slate-200 shadow-sm shadow-blue-900/5 bg-white py-12">
           <Result
             icon={<LockOutlined className="text-slate-300" />}
             title={<Text className="text-xl font-bold text-slate-700">Certificate Locked</Text>}
             subTitle={<Text className="text-slate-500 font-medium max-w-lg mx-auto block mt-2">{lockedMessage}</Text>}
           />
        </Card>
      ) : (
        <Row gutter={[32, 32]}>
          ...
          <Col xs={24} lg={10} xl={9}>
            <Card className="rounded-2xl border-slate-200 shadow-sm shadow-blue-900/5 bg-white h-full relative overflow-hidden"  styles={{ body: { padding: '40px 32px', display: 'flex', flexDirection: 'column' } }}>
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Text className="text-slate-900 font-bold text-base">
                        {certData?.issuedDate ? new Date(certData.issuedDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'N/A'}
                      </Text>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Tag className="m-0 bg-blue-50 border border-blue-200 text-blue-700 font-bold px-2 py-0.5 rounded text-sm">
                        {certData?.finalScore || 0}% Overall Grade
                      </Tag>
                    </div>
                  </div>
                </div>
      ...
      >
        ...
        <div className="bg-slate-900 p-4 flex justify-end gap-3">
          ...
          <Button type="primary" onClick={handleDownload} icon={<DownloadOutlined />} className="bg-blue-600 hover:bg-blue-700 border-0 shadow-sm rounded-lg font-bold text-white">Download Final File</Button>
        </div>
      </Modal>
