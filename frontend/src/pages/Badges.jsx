          ...
          {displayedBadges.map((badge) => (
            <Col xs={24} sm={12} lg={8} xl={6} key={badge.id}>
              ...
              {badge.unlocked ? (
                // UNLOCKED STATE
                <Card
                  className="h-full rounded-2xl border border-slate-200 shadow-sm shadow-blue-900/5 bg-white transition-all duration-300 hover:scale-105 hover:border-blue-400 hover:shadow-lg hover:shadow-blue-900/10 cursor-pointer"
                ...
                // LOCKED STATE
                <Card
                  className="h-full rounded-2xl border border-slate-100 shadow-none bg-slate-50/60 opacity-60 transition-opacity hover:opacity-80"
                  bodyStyle={{ padding: '24px', display: 'flex', flexDirection: 'column', height: '100%' }}
                >
